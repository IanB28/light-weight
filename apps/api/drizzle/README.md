...
- Production additive migration order: create a Neon backup/branch when warranted; run `pnpm --filter @light-weight/api db:migrate`; run `pnpm --filter @light-weight/api db:verify-schema-compatibility`; deploy the API; then verify `/api/ready` and sync before reviewing runtime errors. The verifier is read-only and compares every local journal timestamp plus Drizzle's SQL SHA-256 ledger hash; it does not merely count rows. Never insert Drizzle ledger rows manually and never use `db:push` as the normal production migration mechanism.
- Empty database: use the migration pipeline (`db:migrate`) rather than `db:push` for production. `db:push` is limited to disposable local development environments.
