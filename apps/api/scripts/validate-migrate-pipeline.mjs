...
try {
  try {
    await sql.begin(async (tx) => {
      ...
      assert.equal(firstRun.length, 2, 'first migration run must apply 0000 and 0001');
  ...
  console.log('Drizzle migration pipeline validated: first run applies 0000/0001; second run is a no-op; transaction rolled back.');
} finally {
  await sql.end();
}
