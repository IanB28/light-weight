...
import { normalizeWorkoutSetType, poundsToKilograms, resolveExerciseLoadingProfile } from '@light-weight/domain';
import { Check, ChevronDown, Disc3, Dumbbell, Eye, Trash2, X } from 'lucide-react';
    ...
    <div className={`grid grid-cols-12 items-center gap-1 rounded-2xl p-1.5 transition-[background-color,border-color] ${set.completed ? 'border border-accent/25 bg-accent/15' : 'glass-subcard'}`}>
      ...
      <div className="col-span-4 flex items-center justify-center gap-0.5">
        {!usesAddedWeight ? <span aria-label={t('workout.bodyweightOnly')} className="flex h-11 w-full items-center justify-center rounded-ui-md border border-border-subtle bg-surface-input font-mono text-xs font-bold text-text-muted">BW</span> : <>
          ...
          {weightInputMode === 'plates' ? <PlateWeightButton valueKg={set.weightKg} units={preferences.units} label={t('workout.weightForSet', { set: set.setIndex })} onClick={() => onOpenPlates({ exerciseId, setIndex: set.setIndex, valueKg: set.weightKg, includeBarWeight: session.includeBarWeight ?? loading.includeBarWeight, allowBarToggle: loading.includeBarWeight, loading })} /> : <KeyboardWeightInput valueKg={set.weightKg} units={preferences.units} label={t('workout.weightForSet', { set: set.setIndex })} onChange={(value) => onUpdateSet(exerciseId, set.setIndex, 'weightKg', value)} />}
      ...
      <div className="col-span-2 flex items-center justify-center"><select value={set.rir ?? 2} onChange={(event) => onUpdateSet(exerciseId, set.setIndex, 'rir', Number(event.target.value))} aria-label={t('workout.rirForSet', { set: set.setIndex })} className="h-11 w-full min-w-0 rounded-ui-md border border-border-subtle bg-surface-input px-0 text-center font-mono text-xs font-bold text-text-secondary outline-none focus:border-accent focus:ring-2 focus:ring-accent/25">{[0, 1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>{value}</option>)}</select></div>
...
export function SetTable({ session, preferences, onUpdateSet, onToggleSet, onStartRestTimer, onOpenPlates, onAddSet, onRemoveSet, onUpdateWeightInputMode, onToggleAddedWeight }: SetTableProps) {
  ...
  return <div className="glass-surface space-y-2 rounded-ui-xl border border-border-subtle p-3.5 shadow-card">
    ...
    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 pt-2 text-xs font-semibold"><label className="relative min-w-0"><span className="sr-only">{t('workout.addSetType')}</span><select value="" aria-label={t('workout.addSetType')} onChange={(event) => { const setType = event.target.value as WorkoutSetType; if (setType) onAddSet(exercise.id, setType); }} className="h-11 w-full appearance-none rounded-ui-lg border border-border-subtle bg-surface-input px-3 pr-8 text-xs font-bold text-text-primary outline-none focus:border-accent focus:ring-2 focus:ring-accent/25"><option value="">+ {t('workout.addSet')}</option><option value="working">{t('workout.workingSet')}</option><option value="warmup">{t('workout.addWarmup')}</option><option value="drop">{t('workout.dropSet')}</option><option value="backoff">{t('workout.backoffSet')}</option></select><ChevronDown aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 size-4 -translate-y-1/2 text-text-muted" /></label><Button variant="ghost" size="md" onClick={() => onRemoveSet(exercise.id)} disabled={sets.length <= 1} className="justify-start px-2 text-text-muted hover:text-danger">— {t('workout.removeLastSet')}</Button></div>
  </div>;
}
