...
test('schema compatibility requires every exact local migration hash and timestamp', () => {
  ...
  const ahead = evaluateSchemaCompatibility(expected, [...applied, { when: 1790000000000, hash: 'future-migration' }]);
...
test('compiled compatibility loader resolves the journal independently of process.cwd()', () => {
  ...
  try {
    ...
    assert.equal(journal.at(-1)?.tag, '0006_historical_workout_provenance');
  } finally {
