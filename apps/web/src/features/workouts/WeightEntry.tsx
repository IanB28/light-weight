...
export function PlatePickerSheet({ open, onClose, valueKg, units, baseWeightKg, availablePlatesKg, includeBarWeight, allowBarToggle, loading, onApply }: { open: boolean; onClose: () => void; valueKg: number; units: UnitSystem; baseWeightKg: number; availablePlatesKg: number[]; includeBarWeight: boolean; allowBarToggle: boolean; loading: ExerciseLoadingProfile; onApply: (weightKg: number, includeBarWeight: boolean, baseWeightKg: number) => void }) {
  ...
  return <BottomSheet open={open} onClose={onClose} title={t('workout.platePicker')}>
    <div className="space-y-4 pb-16 sm:pb-0">
      {loading.plateBase?.label === 'smith' && <div className="flex items-center justify-between gap-2 rounded-ui-lg border border-border-subtle bg-surface-input p-2"><span className="text-xs font-bold text-text-primary">{t('workout.smithBar')}</span><div className="flex gap-1" role="group" aria-label={t('workout.smithBarWeight')}>{(loading.plateBase.selectableWeightsKg || []).map((weight) => <button key={weight} type="button" aria-pressed={Math.abs(selectedBaseWeightKg - weight) < 0.001} onClick={() => { setSelectedBaseWeightKg(weight); setHasInteracted(true); }} className={`min-h-11 rounded-ui-md px-3 font-mono text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${Math.abs(selectedBaseWeightKg - weight) < 0.001 ? 'bg-accent text-accent-fg' : 'text-text-secondary'}`}>{formatDisplayWeight(weight, units)}</button>)}</div></div>}
      ...
      <div className="rounded-ui-xl border border-border-subtle bg-surface-elevated p-3" aria-live="polite">
        <dl className="space-y-1.5 text-xs">
          {includesBar && <div className="flex justify-between gap-3"><dt className="text-text-muted">{loading.plateBase?.label === 'smith' ? t('workout.smithBar') : t('workout.bar')}</dt><dd className="font-mono font-bold text-text-primary">{formatDisplayWeight(selectedBaseWeightKg, units)}</dd></div>}
