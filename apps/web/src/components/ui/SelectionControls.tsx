...
export function Chip({ selected, className, ...props }: ChipProps) {
  return <button type="button" aria-pressed={selected} className={cn('min-h-10 shrink-0 whitespace-nowrap rounded-full border px-3.5 text-xs font-semibold transition-[transform,background-color,border-color,color,opacity] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:opacity-45', selected ? 'border-accent bg-accent text-accent-fg shadow-accent' : 'border-border-subtle bg-surface-input text-text-secondary hover:border-border-active hover:text-text-primary', className)} {...props} />;
}
...
export function SegmentedControl<T extends string>({ value, options, onChange, label, className }: SegmentedControlProps<T>) {
  return (
    <div role="group" aria-label={label} className={cn('grid rounded-ui-lg border border-border-subtle bg-surface-input p-1', className)} style={{ gridTemplateColumns: `repeat(${options.length}, minmax(0, 1fr))` }}>
      {options.map((option) => (
        <button key={option.value} type="button" aria-pressed={value === option.value} onClick={() => onChange(option.value)} className={cn('flex min-h-10 min-w-0 items-center justify-center gap-1 rounded-ui-md px-2 text-xs font-bold transition-[transform,background-color,color] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent', value === option.value ? 'bg-accent text-accent-fg shadow-sm' : 'text-text-muted hover:text-text-primary')}>
